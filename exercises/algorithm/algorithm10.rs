...
impl Graph for UndirectedGraph {
    ...
    fn add_edge(&mut self, edge: (&str, &str, i32)) {
        ...
        let mut adj: &mut Vec<(String, i32)> = self.adjacency_table_mutable().get_mut(edge.0).unwrap();

        adj.push((edge.1.to_string(), edge.2));
        adj = self.adjacency_table_mutable().get_mut(edge.1).unwrap();
...
pub trait Graph {
    ...
    fn add_edge(&mut self, edge: (&str, &str, i32)) {
        ...
        let adj: &mut Vec<(String, i32)> = self.adjacency_table_mutable().get_mut(edge.0).unwrap();
